...
# (9 · #359) the boundary-evidence arm. The payload is stubbed through SECOND_SHIFT_BOUNDARY_EVIDENCE
...
# payload path is drift. The payload's own semantics are boundary-evidence-selftest.sh's subject.
EVSTUB="$TMP/boundary-evidence-stub.sh"
cat > "$EVSTUB" <<'EOF'
#!/usr/bin/env bash
echo "[boundary-evidence] stub speaking"
exit "${EV_STUB_RC:-0}"
EOF
chmod +x "$EVSTUB"
...
ev_run() { # ev_run <dir> <stub-rc>
  ( cd "$1" && SECOND_SHIFT_CONFIG_LINT="$STUB" STUB_RC=0 \
      SECOND_SHIFT_BOUNDARY_EVIDENCE="$EVSTUB" EV_STUB_RC="$2" \
