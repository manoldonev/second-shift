...
## Repo-specific network guidance

Use request interception sparingly in committed E2E tests. This repo favors real integration flows against the local app/backend and verifies side effects through services/managers.

Good uses in this repo:

- Capture critical portal checkout payloads while triggering the UI action.
- Assert payment authorization, discount, quantity, or checkout request bodies.
- Debug failing network calls during `--debug=cli`.
- Temporarily block third-party noise while exploring with `playwright-cli`.

Avoid mocking core backend behavior in admin/portal E2E specs unless the scenario is explicitly about an error state that cannot be created through fixtures/services. Prefer setup through `src/services/**` and cleanup through managers.

Portal payload assertion pattern:

```typescript
const [authorizeRequest] = await Promise.all([
  page.waitForRequest((request) => request.url().includes('/checkout/payment/authorize')),
  planCheckoutPage.clickContinueButton(),
]);

const requestPayload = authorizeRequest.postDataJSON();
expect(requestPayload.amount).toBe(246.4);
```

During CLI exploration, use `playwright-cli route` only for temporary diagnosis; call `playwright-cli unroute` when finished.
