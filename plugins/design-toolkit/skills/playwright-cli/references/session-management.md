...
## Sessions vs a repo's committed tests

Committed tests should not rely on manually persisted browser profiles. A well-structured suite creates a fresh browser context per test through a test fixture (optionally with saved auth storage state), then closes the page.

Use named `playwright-cli` sessions for **exploration only** — e.g. `<feature>-<flow>` names that make the session's purpose obvious. For committed specs, express authentication/state through the repo's test fixtures rather than CLI state files.

Do not commit `playwright-cli` storage state files or profile directories. If you save state for debugging, delete it when done.
