...
## Debugging a repo's committed tests

Run the repo's check/build step first, then run the targeted spec with the repo's script and `--retries=0`. If it fails or flakes, inspect the Playwright trace from the command line:

```bash
npx playwright test tests/<area>/<name>.spec.ts --retries=0
npx playwright trace open test-results/<test-name>/trace.zip
...
For interactive diagnosis, launch the specific test with `--debug=cli`, then attach:
...
Debug one file (or one test) at a time, especially for non-parallel suites. Failures involving backend state often need both trace evidence and a check of the created records; visual failures should be diagnosed through trace evidence and screenshot diffs before updating snapshots.

Use traces as the primary agent-readable evidence, but fix tests in the repo's framework: page objects, services/helpers, fixture options, and constants.
...
npx playwright test --debug=cli --retries=0
...
Coding agents should prefer terminal trace inspection with `npx playwright trace` so failing and flaky tests can be understood from the command line.
...
Trace CLI output exposes the action timeline, before/after snapshots, errors, and enough context to diagnose most failures from the terminal.
...
Use the CLI trace tools first. Open the GUI Trace Viewer only when the terminal trace output is insufficient.
