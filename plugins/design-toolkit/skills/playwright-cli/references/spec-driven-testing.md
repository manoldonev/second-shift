...
All three lean on the same mechanic: run `npx playwright test --debug=cli --retries=0` in the background, then `playwright-cli attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics and [test-generation.md](test-generation.md) for how every `playwright-cli` action emits Playwright TypeScript.

## Honoring a repo's test contracts

When the repo ships its own E2E framework, read its E2E guide(s) before planning, generating, or healing tests, and follow these rules even when a generic spec-driven instruction below says otherwise:

- Generate committed tests inside the repo's test directories, matching its layout and file-naming.
- Import from the repo's test fixture, not directly from `@playwright/test`, unless you are writing a temporary scratch script.
- Reuse existing page objects, services/helpers, constants, and preset APIs.
- Run the repo's check/build step before the targeted test command.
- After modifying a test, run the targeted spec with `--retries=0` and iterate until it passes.
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test tests/seed.spec.ts --debug=cli --retries=0
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test <seed-file> --debug=cli --retries=0   # background
playwright-cli attach tw-XXXX
# resume
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test tests/<group>/<scenario>.spec.ts --retries=0
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test --retries=0
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test tests/<group>/<scenario>.spec.ts:<line> --debug=cli --retries=0
...
Edit the test file: update the locator, assertion, step order, or inputs to match the corrected behaviour. Stop the background debug run. Rerun the single test with `--retries=0` to confirm green.
