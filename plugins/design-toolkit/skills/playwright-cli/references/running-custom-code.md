...
## Folding discoveries into a repo's test suite

Use `playwright-cli run-code` for exploration, debugging, and rehearsing interactions. In committed tests, translate useful discoveries into the repo's framework:

- Page-object methods for reusable UI interactions.
- Services/helpers for setup, teardown, and backend reads.
- Fixture overrides for dynamic page objects.

Do not leave large anonymous `run-code` snippets as the mental model for a committed test. Use them to discover stable locators, request payloads, waits, and browser state, then write typed Playwright code in the spec/page object.

Cases where `run-code` is useful:

- Waiting for a hard-to-observe state before deciding what assertion belongs in the test.
- Inspecting localStorage/cookies during auth debugging.
- Reading computed styles or bounding boxes for a visual-regression investigation.
- Capturing a request/response during a failing flow.
