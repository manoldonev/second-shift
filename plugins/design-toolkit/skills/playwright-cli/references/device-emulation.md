...
## Emulation vs a repo's committed tests

For committed visual/device tests, prefer Playwright project configuration (viewports, device presets, test-id attribute) over manual `playwright-cli resize` calls. Use `resize` and the `run-code` emulation snippets below only for exploration or quick debugging.

When adding visual coverage, follow the repo's conventions: stabilize timezone and clock when dates affect the UI, scope device projects (desktop vs mobile) to the surfaces each actually supports, and reuse the repo's setup helpers for any cookies/state a screenshot needs.
