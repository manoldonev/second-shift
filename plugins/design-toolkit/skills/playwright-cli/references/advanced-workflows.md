...
## Folding advanced flows into a repo's test suite

"Advanced" E2E workflows are usually CRUD flows needing page objects plus services for cleanup, mutation flows needing dynamic IDs / unique data / request-payload assertions / backend-state cleanup, or visual-regression flows needing deterministic route params, a stable clock/timezone, and screenshot masking of unstable UI.

Prefer the repo's existing framework over one-off scripts — its test fixture, seeded-data constants, and cleanup helpers. When exploring with `playwright-cli`, capture the flow and generated locators, then implement the durable version in the repo's page objects/specs and run the targeted spec with `--retries=0`.
