...
## Working with a repo's committed test suite

If the repo has its own Playwright/E2E framework, prefer it over ad hoc CLI runs: read the repo's E2E guide(s) and run tests through the repo's package scripts, which set the expected config and base URLs. Run the repo's install/build step before a modified spec, and pass `--retries=0` when an agent runs the tests so each failure is reported once and the first error is not obscured by retries. Reach for raw `npx playwright test` only when you need `--debug=cli`.

Inspect a failing/flaky test trace from the command line:
...
When using `playwright-cli` to explore the app, mirror the same environment/auth state the spec will use, and start from the repo's dev-server URL.
