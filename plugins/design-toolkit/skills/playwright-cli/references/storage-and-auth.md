...
## Auth in a repo's committed tests

For committed tests, prefer the repo's E2E fixture system over hand-written `state-save` files:

- Authenticated tests should acquire and reuse saved auth storage state through a fixture (log in once, persist the state, reuse it across tests) rather than re-driving the login UI per test.
- Tests for unauthenticated flows should start from a clean context, then assert any created backend state through the repo's services/helpers.

Use `playwright-cli state-save` only for temporary local debugging. Never commit saved auth state files.
