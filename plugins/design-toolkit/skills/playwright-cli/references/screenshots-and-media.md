...
## Screenshots vs a repo's visual-regression tests

For committed visual-regression coverage, use the repo's visual-test framework (its config, spec location, and screenshot scripts) instead of ad hoc `playwright-cli screenshot` artifacts. Follow the repo's standard screenshot options, e.g.:

```typescript
const visualTestOptions = {
  fullPage: true,
  threshold: 0.0,
  maxDiffPixels: 5,
};
...
Stabilize visual tests with explicit route params, a stable timezone, a fixed browser clock, visible-content assertions before screenshots, and hiding uncontrolled UI (maps, live data) where the repo's existing tests do so.

Use `playwright-cli screenshot` for debugging or user-visible evidence only; do not treat it as a replacement for `toHaveScreenshot()` in committed visual specs.
