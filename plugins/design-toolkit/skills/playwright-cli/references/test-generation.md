...
> **When to use**: Generating Playwright test code for this repo from interactive CLI sessions — recording user flows, building test scaffolds, converting manual testing into automated tests.
> **Prerequisites**: [core-commands.md](core-commands.md) for basic CLI usage

## Generation rules

Do not generate a test from the scenario text alone. Explore the real app with `playwright-cli`, complete the flow step by step, then write the Playwright TypeScript test from the observed behavior and generated locators.

When the repo ships its own E2E framework, follow it: read its E2E guide(s), import from its test fixture (not directly from `@playwright/test`), put specs in the directories its layout dictates, reuse its page objects and services/helpers for setup/teardown/backend assertions, and run the targeted spec with `--retries=0` until it passes.

Separate functional specs (interaction + assertion) from visual specs (deterministic clock/timezone/route params + `toHaveScreenshot()` with the repo's screenshot options).
