"""
Rubric for the figma-faithful-plan-reviewer agent eval (3-dimension / 10-point scale).
...
Design notes
------------
#704's acceptance criteria grade in BINARY ("must Block", "must not Block") while the kit scores
a weighted rubric, so the verdict dimension carries the bulk of the points and the quality
dimensions carry the rest. That keeps "fixtures 1-3 pass >= 3/3 runs" readable straight off
`per_fixture` in the results JSON, while `overall_pct` still feeds the +10pp/3-run rule.

- d1_verdict_correctness is worth 6 (the top-line call, and the only dimension #704 states).
...
Total: 10. Aligns with the plan-reviewer, security-reviewer and intake-orchestrator campaigns.
"""
...
JUDGE_SYSTEM = """You are scoring a figma-faithful TRANSLATION-PLAN review against a 3-dimension rubric.
...
- <fixture_content>: the translation plan the reviewer was asked to review.
...
d3_no_fabrication (0, 1 or 2) -- no findings against content the plan does not contain.
  ...
  1 if exactly one finding is fabricated or contradicts `must_not_flag` (e.g. flagging a token
    row as wrong arithmetic when it is correct, or asserting a dimensions table is empty when it
    is populated).
