...
1. First `git fetch origin <base>`, then run `git diff origin/<base>...HEAD --stat` to understand the scope of **committed** changes. Resolve `<base>` = the user's specified base if given, else the remote's default branch (`BASE=$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null | sed 's|^origin/||'); BASE=${BASE:-$(git rev-parse -q --verify origin/main >/dev/null && echo main || echo master)}`) — a hardcoded `main` diffs against the wrong ref on a develop/alpha-based repo.
