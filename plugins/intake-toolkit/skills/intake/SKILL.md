...
| Unstructured **bug blob** (vague STR, maybe-wrong reporter comments)    | `intake-interviewer` (bug mode)              | emit issue → `plan-interview` (often explicit-empty ledger) → implement/`/dev-pipeline:run-lean`  | reproducibility exit criterion; reporter-owned facts stay `Unknown` over inference                                                                         |
...
| GitHub **issue logged by an engineer** (story/bug)                      | `plan-interview` pre-flight                  | implement / `/dev-pipeline:run-lean`                                              | spec presumed implementable; ledger captures design decisions only (explicit-empty and go if none are material)                                            |
