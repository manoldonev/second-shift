...
- **`Skill()` loads are visible, but this aggregator does not count them.** Programmatic skill loads (`/dev-pipeline:review → review-lead`) *do* reach the ledger as `PostToolUse` rows with `tool: "Skill"`, and `target` carries the skill name. The "top loaded orchestrators" tally below is built from `UserPromptExpansion` / `command_name` only, so it still reflects user-typed slash commands alone. For nested loads, query `target` directly — see the audit `QUERIES.md`.
