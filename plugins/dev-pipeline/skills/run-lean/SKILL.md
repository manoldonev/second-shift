...
9. Post one closing comment: PR link, verdict-record reference, same cost block. Then `bash G 5 <issue>` — exit artifacts. Remove the worktree, but **leave the claimed label alone**: milestone 5 requires an open PR, so review is still in flight and the label is correct. It is released when the item closes, by the repository's unclaim workflow.
