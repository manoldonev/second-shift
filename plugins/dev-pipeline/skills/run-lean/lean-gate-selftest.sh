...
if [ "$(jq -r '.tracker.branchPrefix // "absent"' "$CFG_NOPREFIX" 2>/dev/null)" = "absent" ]; then
  ...
  if [ "$rc" -eq 0 ] && grep -q '^branch_prefix: unresolved' "$WORK/p3.md" 2>/dev/null; then
    pass "(e5) milestone 1 still evaluates with an unresolvable prefix, and records it as unresolved"
  else fail "(e5) expected milestone 1 to pass and record 'unresolved', got $rc: $out / $(grep '^branch' "$WORK/p3.md" 2>/dev/null | tr '\n' ' ')"; fi
