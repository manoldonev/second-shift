...
2. If `status: "in_progress"` — resume from the `currentStage`. Print a one-line summary of what was already completed. **Then record this session for cost attribution**, before doing any stage work:
   ...
   A resumed run is a **different** Claude session from the one the original run recorded (at Stage 1, then again at Stage 2), and Stage 9's cost block attributes cost by `session.id` against `pipelineSessions[]`. Recording only at the run's start and at the Stage-8 crash-recovery entry would mean a resume that re-enters at any other stage (7, 6, 5, …) contributes **zero** rows to the cost block — its entire cost silently vanishing rather than showing up as a gap; hence this call on **every** `in_progress` resume, whatever stage it lands at. `pipeline-session-add` is idempotent on the session id and is deliberately exempt from the terminal-state guard, so this is safe on every resume path, including a post-terminal backfill. It is also the join key the **ledger-corroboration** legs resolve the audit ledger through (`state-schema.md`), so a resume that skipped it would complete its stages `uncorroborated`.
