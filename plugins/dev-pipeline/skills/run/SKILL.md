...
**Recovery from a pre-flight abort is a hard handoff, not an in-place continue.** After the operator fixes the failed prerequisite (installs the bot wrapper, creates a missing label), the run does **not** resume from where the abort stopped. Re-invoke `/dev-pipeline:run <n>` from the top so every early step runs in order — the **Pre-flight: Generate RUN_ID** step and Stage 1.A's atomic claim. An agent that continues in-place after an abort silently skips these setup steps and takes a control-flow decision that belongs to the operator. (Cost attribution is no longer among the steps that can be skipped this way: registration is owned by statectl's write seam, so the session registers itself on its first state write regardless of where the run re-enters. The historical failure this warning was written for — a skipped Stage-2 session record surfacing as a `skipped-no-sessions` cost block — is closed by construction.) If continuing in-place is genuinely unavoidable, surface the choice to the operator first — never take it silently.
...
The `code-review` receipt carries one extra condition: `comment-add` refuses it until `review-toolkit:review-lead` is recorded in `stages.8.skillsLoaded[]`, so the skill that governs synthesis must be loaded **before** the synthesis is authored — not afterwards to clear the Stage-8 completion gate. Load it, record it, then synthesize and post.
...
**Non-base-branch posture:** a current branch other than the configured base is **not** a reject — Stage-1 reads are pinned to `origin/<baseBranch>` (stages/1-intake.md Step 1.P) and Stage 2 cuts the work branch from the same remote ref, so the checkout's branch cannot leak into the run; merge and index state is per-worktree, so `worktree add` neither reads nor disturbs it. The predicates: pin established + clean tree → proceed silently; **dirty working tree — including an in-progress merge or rebase, conflicts and all** (any branch) → surface a WARN — "a human appears to be mid-work in this checkout" — in the run's final report and proceed; **pin not establishable** (fetch/worktree-add failure) → fail closed via `mark-failed --reason non-main-base-autonomous` (interactive mode presents the failure instead). An in-progress merge or rebase is never a stop: it rides the dirty-tree predicate whenever it leaves changes behind, and the clean predicate when it does not. Wrong-target detection (wrong repo, wrong issue, wrong diff base) is unchanged and still aborts.
...
- **After Stage 1 (Intake):** crash-recovery only. Carry forward: issue number, verdict, decomposition decision, key file paths from codebase-explorer.
...
| `currentStage == 7` AND `stages.7.status == "completed"`                                             | Crash-recovery resume only (an interrupted run re-invoked in a fresh session — the happy path flows in-process from Stage 7 into Stage 8). Skill enters the Stage 8 crash-recovery resume entry: validate worktree, hydrate from `stageCheckpoint["7"]`, dispatch reviewers via the Stage 8 `Workflow` script (`workflows/code-review.mjs`), then run review-toolkit:review-lead synthesis in-session. The pause span needs no step — the shared write seam records it on this session's first state write, whichever subcommand that is. |
...
**Required file shape** (`mark-completed` refuses a terminal write without it — write the eval BEFORE `mark-completed`): parseable JSON with `ticketKey` = the issue key **as a string** (e.g. `"98"`) and a `criteria` object scoring **exactly** the five locked keys from `eval-criteria.md`'s example — `target_confirmation`, `plan_grounding`, `implementation_resilience`, `scope_compliance`, `review_precision` — each valued `PASS` | `FAIL` | `N/A` (nothing else; `mark-completed` rejects renamed/missing/extra keys and any other value, naming the offenders). Everything else (outcome, notes, evidence) is free-shape. When self-scoring `scope_compliance`, cross-check the **committed file list** against the plan's Affected files — not just the AC verdict (a retro FAIL class: out-of-plan commits graded PASS off the scope-reviewer alone).

**This self-score is a floor, not the record.** The executor grading its own run is structurally generous; the operator should follow up with **`/dev-pipeline:pipeline-retro <issue-number>`** (sibling skill, [`../pipeline-retro/SKILL.md`](../pipeline-retro/SKILL.md)) — it re-scores the five criteria with a fresh-context agent from artifacts only (evidence quotes required), audits the run for contract deviations (silent deviations are the headline metric), logs environment friction, and routes every finding to a skill edit, issue, doctor check, or criteria proposal. Its cross-run sibling **`/dev-pipeline:perf-retro`** ([`../perf-retro/SKILL.md`](../perf-retro/SKILL.md)) covers the other axis — it profiles execution latency across the recorded run corpus and proposes optimizations that must each name an existing regression guard, so speed never arrives by weakening a gate.
