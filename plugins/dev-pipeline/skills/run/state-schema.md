...
- `crossBoundaryReviews` — **be-fe-pair dual-target only (#48), additive.** Array of per-repo Stage-8 review records for a **secondary** target repo (the one that is not the in-session review repo). Each entry `{ repo, status, worktreePath?, baseSha?, headSha?, handoffEmittedAt?, note? }`: `status: "completed-in-session"` when the reviewer plugin was loadable and the secondary repo was reviewed in-session, or `"pending"` when it was emitted as a non-blocking interactive **handoff** (Stage 9 surfaces pending handoffs as PR "review pending" bullets). Absent on every single-target / non-pair run. A non-empty array satisfies the Stage-8 completion precondition on its own (the primary repo's review still records `codeReviewRounds`). Producer ships in a later phase; the Stage-8 completion gate already honors it.
- `skippedReviews` — **be-fe-pair dual-target only (#48), additive.** Array of `{ repo, reason }` for a target repo that was neither reviewed nor handed off (e.g. no reviewer available). Like `crossBoundaryReviews`, a non-empty array satisfies the Stage-8 completion precondition. Absent otherwise.
