...
- `stages` — map keyed by stage number. Each entry tracks:
  ...
  **Completion-evidence preconditions (imperative stage machine).** `set-stage N --status completed` is refused (`EXIT_CODE=1`) unless the state carries the evidence that stage N's mandated work actually happened — deterministic state reads only, with conditional applicability from the flags the run itself recorded. `--force` bypasses these (crash-recovery escape, same as the monotonic guard):
  ...
  | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | 1     | `stageCheckpoint["1"]` is an object **carrying a well-formed `preflight` `{baseBranch: non-empty string, workingTreeClean: boolean, guardOutcome: non-empty string}`** (the pre-flight attestation — see below; `workingTreeClean:false` is valid) |
  ...
  | 8     | `codeReviewRounds >= 1` **or** a non-empty `crossBoundaryReviews[]` **or** a non-empty `skippedReviews[]` (the be-fe-pair dual-target escape hatch, #48 — a secondary repo reviewed via handoff or explicitly skipped completes the stage without a primary-worktree review round) |
