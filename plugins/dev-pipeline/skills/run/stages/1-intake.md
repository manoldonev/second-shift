...
**Then assert the session id — immediately after `init`.** The `init` write has *already* registered this session in `pipelineSessions[]` via statectl's shared write seam, so this call is **not** what populates the join key the ledger-corroboration legs read. It is here for the one thing the seam deliberately does not do: the seam **declines silently** on a session id that is set but not a UUID (`is_session_uuid || return 0`), whereas `pipeline-session-add` **rejects** it. Running it here therefore turns a broken harness environment into a loud Stage-1 failure instead of a run that proceeds silently uncorroborated:
...
The `-n` guard is the same one Stage 2 carries: an **unset** id is a legitimate non-recording environment — it records no join key and every stage completes visibly `uncorroborated` rather than silently. A **set-but-malformed** id is not legitimate, and hard-failing on it at the write is the intended posture. The call is idempotent on session id, so on the normal path it is a no-op over the seam's own record (it only upgrades that record's `source` from `null` to `interactive`).
...
# fetch-then-pin idiom). WORKTREES_DIR resolves via tools/resolve-worktrees-dir.sh
