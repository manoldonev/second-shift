...
**Env hermeticity.** Every configured lane command (`setup` lanes, `format`, `lint`, `type-check`, `test`, the `lint --fix` autofix + recheck, `extraLanes`) runs as a child process with the pipeline's own seam vars (`SECOND_SHIFT_CONFIG`, `SECOND_SHIFT_REPO_ROOT`, `STATECTL_STATE_DIR`, and related overrides — the full denylist is verifyctl.sh's `SEAM_SCRUB`) scrubbed via `env -u`. A consumer's configured lane can itself be second-shift tooling (dogfooding this repo's own `test` command is its selftest sweep), and an ambient pipeline value silently re-roots or re-states it, producing spurious failures unrelated to the code under review (#34, #83). `PATH` and the selftest-only `VERIFYCTL_TEST_MARKERS` are deliberately never scrubbed.
...
`INFRA` entries in `failures[]` take the classification table's INFRA row below (a `packages-build` or `install` failure is INFRA-grade: a build-order/environment prerequisite, not a verify fix loop). On `status: "pass"`, persist the verify summary — **both lanes**, verifyctl's skip strings included (the INERT skipped-string, the zero-lane `allowUnverified` opt-out string, the when-gated extraLanes-miss string):

**Verdict-honesty contract.** The object summary's lanes initialize `skipped` and are promoted on execution — a lane that did not run never reports `clean`/`passed`; `setup` (renamed from the former `build` field) reports the `lanes[]` setup outcome. `set-stage 6 --status completed` additionally enforces a content gate on object summaries: at least one verifying lane (`lint`/`typeCheck`/`test`/`ext:*`) must have actually run (`format`/`setup` never satisfy it; absent keys fail). A zero-lane repo either sets `commands.<repo-id>.allowUnverified: true` (verifyctl then emits the opt-out string — only when nothing failed) or verifyctl itself now fails closed (#115: `status: "fail"`, a `CONFIG` entry in `failures[]`, never charged) rather than emitting an optimistic all-skipped object for the Stage-6 gate to catch — the two layers agree instead of only the pipeline-layer gate refusing after the fact. `commands.<id>.build` remains accepted-but-unexecuted config for now.
...
**The summary alone no longer closes the stage.** That same precondition also requires verifyctl's runId-scoped **sidecar** — `{issue}-verify.json`, or `{issue}-<id>-verify.json` for every target under `--repo` — to exist and carry this run's `runId`. It is the attestation that the suite was actually RUN rather than merely summarized: `verify-summary-set` records whatever JSON the session hands it, so a hand-composed summary previously reached a completed Stage 6 that was indistinguishable in state from a real verifyctl run. Running the suite through `{verifyctl}` as this section already mandates satisfies the gate automatically — the only way to trip it is to have bypassed verifyctl. Absent and stale-`runId` sidecars are refused with distinct messages, both naming `verifyctl.sh`; a reason-carrying `--force` from an attended shell (`DEV_PIPELINE_MODE=interactive` on a pipeline-owned state — #243) is the sole bypass, recorded as a waiver, as with every neighbouring precondition.
...
A changed file is INERT iff it matches the effective pattern set, which resolves in [`tools/is-inert-diff.sh`](../tools/is-inert-diff.sh) (the single source of truth). The **default** set is `*.md` · `*.sh` · `.github/workflows/*.yml` · `.claude/**/*.{mjs,cjs,py,tsv,json,jsonl}` · `.claude/second-shift/.known-extensions` · `.prettierignore` · `.gitignore`. Any non-matching path — `.ts/.tsx/.js/.json` outside `.claude/`, lockfiles, `package.json`, config, CSS, Python, Rust, anything new or unrecognized — selects the SUITE lane (default-to-SUITE when in doubt; never widen the inert list mid-run). verifyctl re-derives the lane on every invocation, so a fix commit that introduces a non-inert file automatically flips the lane. The per-pattern rationale (why each carve-out is zero-coverage, and how narrowly each is anchored) lives as comments above the pattern set in that script.

**That default is JS/TS-centric, and config `stageParams.inertPattern` overrides it.** The enumerated set above is inert because a JS/TS lint/type-check/test suite gives those paths zero coverage — which stops being true when they ARE the product. On a shell-and-Markdown repo the default classifies every real diff inert, so the configured `lint`/`test` lanes never run and this stage reports a false green. Such a consumer sets `stageParams.inertPattern` to an ERE that **replaces** the default outright; verifyctl resolves the key and passes it to the classifier. Three properties worth knowing:

- **Replace, not merge.** Only replacement can *remove* an alternative such as `\.sh$`. The consequence is that an override is a hand-copy and will not inherit future additions to the shipped default.
- **Fail closed.** An uncompilable pattern is rejected by `config-lint.sh` before a run; if one reaches the classifier anyway it selects SUITE with a stderr diagnostic, never INERT. Widening the inert set is the only dangerous direction — over-running the suite is waste, a skipped verification is a false green.
- **`stageParams` is global, not per-repo.** In a `be-fe-pair` (or any multi-repo) topology, `verifyctl --repo <id>` verifies one target at a time but resolves this single global value for every one of them. A pair whose repos have genuinely different product surfaces — a TS front end beside a shell/Python back end — cannot express that here: the pattern must be the union that leaves BOTH surfaces non-inert, which is the safe direction (it over-runs rather than skips). A per-repo override would belong under `topology.repos.<id>`, and is deliberately not introduced until a real consumer needs it.

`preflight.sh` warns when the effective pattern makes a repo's configured lanes unreachable — i.e. when every tracked file classifies inert — so the condition surfaces before a run rather than as a mystery "skipped (inert diff)" after one.

**Selecting SUITE brings more than lint/test.** A diff flipped from INERT to SUITE also runs the setup `lanes[]`, any `extraLanes`, and switches format from a scoped `prettier --check` to `prettier --write`. A consumer that leaves `commands.<host>.format` absent gets the prettier default, and with no local binary the `npx` fallback's rc 126/127 is an `INFRA` hard stop — so set `format: null` if the repo has no formatter.
...
| `INFRA`            | Command not found, OOM, timeout, packages-build/install failure | Surface to user immediately and fail fast — not auto-fixable, **never charged, never retried**. | never       |
| `CONFIG`           | Zero verify lanes configured (`lint`/`typeCheck`/`test`/`extraLanes` all absent) and no `allowUnverified` opt-out (#115) | Not a fix loop — configure a verify lane for the repo, or set `commands.<host>.allowUnverified: true` if it genuinely has none. | never       |

**Retry budget:** Max 2 fix attempts **per failure class**. The four suite classes are verifyctl-charged per the table above (a re-invocation after a failed run IS the fix attempt); `PLAN_CMD_FAILURE` is charged in-session via `statectl verify-attempts "$ISSUE_NUMBER" --incr PLAN_CMD_FAILURE` on each fix attempt, compared against the same budget.

**Plan-specific verification commands are in scope — on BOTH lanes.** The plan's "Verification commands" section (selftests, regenerate-and-diff checks, custom scripts) runs **in-session** as part of this stage, after verifyctl passes; the inert lane never skips them. A fix loop on them charges `PLAN_CMD_FAILURE` (tool-missing/environment errors stay `INFRA`) — the one in-session carve-out from verifyctl's counter ownership. Running them earlier (during Stage 5) is fine for fast feedback, but a fix loop on them is a verify fix loop regardless of which stage discovered it: track it at that moment.
...
**Safety-net re-verify (one shot).** Run `{verifyctl} run {issue-number} --no-attempt` — the `--no-attempt` contract is load-bearing: no budget check, no `verifyAttempts` increment, no sidecar write (read-only accounting posture; a red safety net followed by the reset leaves accounting byte-identical to the pre-cleanup state). Because it writes no sidecar, a `--no-attempt` run **does not attest** for the completion gate above — the attesting artifact is the last *charging* `{verifyctl} run`, which always precedes this pass. Do not "fix" that by making `--no-attempt` write one: the byte-identical-accounting contract is the whole point of the flag.
