...
- **Downstream is unchanged:** the delegate's commits are ordinary Stage-5 commits. They flow through the normal Stage-5 completion (the scope-enforcement gate, the unit-test mutation review, the checkpoint) and then Stage 6 verify + Stage 8 review **exactly as inline commits do**.
...
2. **Live-render verify gate.** `statectl stage-substatus "$ISSUE_NUMBER" --stage 5 --key designPlanReview --value verifying`. The gate is armed by config, not by hope: `LIVE_RENDER=$(jq -c '.design.liveRender // empty' "$SECOND_SHIFT_CONFIG")`.
   ...
   - Record the outcome either way, in THREE places (#243): the Stage-5 comment; the detail-bearing checkpoint record `renderVerify: { status: "verified" | "degraded", detail? }` (the human/retro record, unchanged); AND the gate-transport sub-status derived from its `status` — `statectl stage-substatus "$ISSUE_NUMBER" --stage 5 --key renderVerify --value <verified|degraded>`. The Stage-5 completion precondition reads the SUB-STATUS only (it survives a crash-recovery resume that skips this gate when `designPlanReview == implemented`); no one cross-checks the two records.
