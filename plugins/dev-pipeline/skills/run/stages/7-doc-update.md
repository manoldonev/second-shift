...
1. **Post the issue comment** via `bash "${CLAUDE_PLUGIN_ROOT}/skills/run/tools/gh-bot.sh" issue comment`: `stage: doc-update, status: completed`. Record the receipt: `"$STATECTL" comment-add "$ISSUE_NUMBER" --marker doc-update --url <html_url>` — Stage-7 completion refuses without it.
