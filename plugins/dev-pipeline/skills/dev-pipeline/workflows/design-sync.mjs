export const meta = {
  ...
  description:
    "design-faithful produce/gate engine for the dev-pipeline. kind='produce' dispatches the " +
    'design-faithful-spec skill to write a acme FE spec artifact (implement:true → the ' +
    'design-faithful skill, which writes apps/web code + commits in-session); kind=\'gate\' ' +
    'dispatches the FE review agents (design-faithful-reviewer / a11y-reviewer) in parallel and ' +
    'returns their trinary verdicts. Results are normalized into the same envelope shape as the ' +
    'review fan-out (code-review.mjs). Aggregate gate-verdict synthesis is NOT done here — it ' +
    "stays in the caller's session (#199 integration), mirroring code-review.mjs.",
...
if (kind === 'produce') {
  ...
  const prompt =
    `Load the ${skill} skill. Open the Claude Design handoff project by id \`${projectId}\` via the ` +
