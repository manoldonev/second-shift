...
**Design-driven runs (interactive only).** An issue carrying a `claude.ai/design/...` handoff link runs `designDriven`: Stage 1 detects + records it, Stage 3 produces a faithful FE spec via the `design-sync.mjs` engine, Stage 5 implements the screen in `apps/web` via the engine + a live-render verify gate, and Stage 8 routes the `design-toolkit:design-faithful-reviewer` + `review-toolkit:a11y-reviewer`. The handoff read goes through the **DesignSync** tool, which needs interactive auth (design scopes; the project is opened by id) — so **run a design-driven issue interactively, not headless**. A headless run has no DesignSync access and fails closed `design-source-unreachable` at the Stage-1 reachability probe (it does not guess a contract). The mode is off by default: an issue with no design link runs exactly as before. Full state contract: state-schema.md **Design Mode**.
...
3. If `status: "failed"` — resume from the failed stage. Print the failure context.
...
| Bug/chore with true spec blockers                      | Comment + stop                                                           | Not created | → `needs-spec-work`                                                          |
| Orchestrator uncertain / threshold exceeded            | Comment analysis + stop                                                  | Not created | → `needs-intake-review`                                                      |
| Sub-issues: decomposition complete                     | Create sub-issues, stop                                                  | Not created | Parent → `epic` (remove `in-progress`, unassign), children → `ready-for-dev` |
| Spec has resolvable gaps only (≤5)                     | Resolve gaps, continue                                                   | Created     | `in-progress` stays                                                          |
| Too many resolvable gaps (>5)                          | Comment + stop                                                           | Not created | → `needs-spec-work`                                                          |
| Spec has true blockers                                 | Comment + stop                                                           | Not created | → `needs-spec-work`                                                          |
