...
**Apply + commit.** If no cleanups are warranted, record `{runId, status: "completed", outcome: "no-candidates"}` and move on — no commit, no re-verify (zero cost; the common case). Otherwise apply the cleanups, run the scoped prettier on the touched files (discard unrelated-file formatting), and land **at most one commit**: `refactor(scope): <summary>` via bot identity. If the pre-commit `type-check` hook rejects the commit, discard the edits (`git -C "$WT" checkout -- .`) and record `outcome: "reverted"` — no retry. Immediately after a successful commit, record `{runId, status: "running", outcome: "applied-unverified", commitSha: "<sha>"}` — the sha makes crash rollback deterministic.
