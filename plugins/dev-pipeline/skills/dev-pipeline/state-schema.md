...
`designDriven` runs wire the **design-toolkit:design-faithful** capability (epic #193 — engine `workflows/design-sync.mjs`, skills `design-toolkit:design-faithful` / `design-toolkit:design-faithful-spec`, reviewers `design-toolkit:design-faithful-reviewer` / `review-toolkit:a11y-reviewer`) through the pipeline. A run is design-driven when the issue body carries a `claude.ai/design/...` handoff link. The mode is **off by default** — a run with no such link behaves exactly as before, and none of the fields below are written. DesignSync (the handoff read) is an interactive, model-invoked tool (design scopes; project opened by id) — so a design-driven issue is expected to run **interactively**; a headless run fail-closes `design-source-unreachable` at the first design read (see `SKILL.md` launch note).

- **`stageCheckpoint["1"].designDriven`** — boolean. `true` when Stage 1 detected a handoff link and the reachability probe succeeded; `false` (or absent) otherwise. Stages 3/5/8 branch on it via `statectl get <issue> '.stageCheckpoint."1".designDriven'`. It lives inside the free-shape Stage-1 checkpoint manifest (no statectl validator — same posture as `intakeMode`), so no schema-enum change is needed to add it.
- **`stageCheckpoint["1"].designSource`** — `{ link, projectId, screen }` (or `null` on non-design runs). The DesignSync handoff coordinates Stage 3/5 pass to the `design-sync.mjs` engine (`projectId` opened **by id**; `screen` is the screen/component to spec + implement, e.g. `detail`).
