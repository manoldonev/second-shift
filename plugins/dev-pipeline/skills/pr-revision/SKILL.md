...
# Use bash "${CLAUDE_PLUGIN_ROOT}/tools/gh-bot.sh" instead of gh for: api POST/PATCH/PUT, pr comment, issue comment, issue edit
...
- Add `in-progress` label to issue (if issue found): `bash "${CLAUDE_PLUGIN_ROOT}/tools/gh-bot.sh" issue edit $ISSUE_NUMBER --add-label in-progress`
