---
name: review
description: The REVIEW half of the pipeline — review an open pipeline PR from a fresh top-level session and produce the committed verdict record. Runs outside the build session by design; the build harness cannot produce this record.
---
...
2. `gh pr view <pr> --json number,headRefName,baseRefName,body,url` — the head branch resolves
   the issue key (`Closes #N` in the body) and the committed spec path.
...
6. Write the record **from the checkout of the PR head**:
   ...
   usable result from — whether from the fan-out or from a `review-toolkit:review-lead` Step 4b
   re-dispatch** — qualified as `review-toolkit:review-lead`'s own plugin-shipped panel names them
