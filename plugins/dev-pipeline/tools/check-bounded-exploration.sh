...
#        Default dir is the sibling workflows/ of this script's plugin root.
...
# was a real hole rather than a theoretical one while the pipeline shipped its own workflow: its
# structured-emitter fallback IS a schema-carrying dispatch. The review half moved to a
