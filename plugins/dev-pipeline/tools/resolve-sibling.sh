...
# at a DIFFERENT depth (milestone-gate.sh, #562) reuses the ladder instead of re-deriving it.
#
...
# milestone-gate.sh sits two), and that arithmetic stays the CALLER's, computed the same way
...
# `. .../resolve-sibling.sh` for the pattern, and milestone-gate.sh's resolve_ledger_lint() for a
