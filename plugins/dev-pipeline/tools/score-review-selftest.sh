...
grep -qE '(^|[[:space:]])(const|let|var)[[:space:]]+dispatchSchemaAgent' "$PLUGIN_DIR/workflows/code-review.mjs" \
...
grep -q "Copied verbatim from code-review.mjs" "$PLUGIN_DIR/workflows/stall-probe.mjs" \
...
[[ -e "$SCRIPT_DIR/tracker/jira/README.md" ]] \
