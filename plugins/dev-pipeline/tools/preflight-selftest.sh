...
# <cache>/<marketplace>/dev-pipeline/<version>/tools, so it resolved to nothing,
...
# cache version carrying the marker. The ANCHOR is this plugin's root (the caller walks up to
# it), so the hop constants are one to the plugins dir and two to the marketplace root — the
# same on both sides of the lockstep pair regardless of how deep either tool directory sits.
# Newest version is chosen LEXICALLY, mirroring the house ladders: `9.0.0` outranks `10.0.0`,
# a shared latent defect deliberately mirrored rather than fixed here.
#
