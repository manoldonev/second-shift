...
# (1b) DROPPED in #348. Its subject was `stages/6-verify.md` delegating the inert-lane
#      decision to is-inert-diff.sh rather than re-inlining the grep; the staged lane and
#      that doc are gone. It is not re-anchored at a surviving consumer, because every
#      remaining one mentions the script in PROSE — asserting a markdown file contains a
#      basename is a prose-presence guard (CLAUDE.md forbids adding them), and it could not
#      fail for a reason a reader of the diff would not already see. Case (1) above still
#      pins the load-bearing half: the hook carve-out and the classifier's inert set stay
#      in lockstep.
