...
10 merged **lean** PRs' own descriptions, reading `cost_usd:` first and falling back to a labeled
legacy `$N.NN` block-grep for a PR that predates this ticket (no key at all). "Lean" is decided by
`headRefName` carrying the repo's own configured `tracker.branchPrefix` (D-4) — a plain `gh pr list
--state merged` window is NOT this, since release PRs and any other non-lean merge sit in the same
window and can never carry the key, biasing the coverage line low:

```bash
BRANCH_PREFIX="$(jq -r '.tracker.branchPrefix // ""' .claude/second-shift.config.json)"
gh pr list --state merged --limit 60 --json number,mergedAt,body,headRefName | jq -r --arg prefix "$BRANCH_PREFIX" '
  def classify:
