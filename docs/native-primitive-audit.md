...
| --- | --- | --- | --- |
| Worktree plumbing (`../<repo>-worktrees`, branch naming, cross-repo pairs) | Native worktree isolation (EnterWorktree / `isolation: worktree`) | **Defer (v2), verify live** | Native worktrees are session-scoped and auto-cleaned; the pipeline needs named, persistent, resumable worktrees shared across sessions and repo pairs. Re-evaluate once native worktrees support naming/persistence. |
...
| `pre-commit-typecheck.sh` PreToolUse hook | Native `/verify` skill, hooks system | **Keep** | Hooks are the native mechanism; the script just becomes config-driven (typecheck command per repo). `/verify` complements the verify lane, doesn't replace commit gating. |
