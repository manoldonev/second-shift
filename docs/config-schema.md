...
| `gates` | `mutation` — defaults off; `false` is an explicit off-switch declaring the mutation seam deliberately absent. The lean lane's sweep is repo-carried (`tools/mutation-sweep.sh`, run by milestone 3 when the repo ships one), so this key is the declared intent config-grill grades that plumbing against rather than a switch the gate reads | declaring no-mutation-coverage deliberately on a repo with no sweep |
