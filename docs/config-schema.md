...
- **Exit code `3` is RESERVED on a verify lane: "this failed for reasons that are not the branch."**
  ...
  `milestone-gate.sh` and reds when it and this list disagree — which is what the list is for. The
...
- **`ticketTag` is advisory, and only advisory.** It keys off
  ...
  staged lane, so only the advisory reading remains — no gate reads it, `milestone-gate.sh`
