...
| `gates` | `mutation` — defaults off; `false` is an explicit off-switch declaring the mutation seam deliberately absent. The sweep is repo-carried **and repo-run** (`tools/mutation-sweep.sh`, wired on your own merge boundary; no milestone-3 lane runs it), so this key is the declared intent config-grill grades that plumbing against and has never been a switch any gate reads | declaring no-mutation-coverage deliberately on a repo with no sweep |
