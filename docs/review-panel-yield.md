...
| P-3 | `security-reviewer` | **demote** to surface-conditional | 41 dispatches, 2 upheld blockers — both on #647 (`dea10754`), the one PR in the window touching a permissions surface | Auth / tenancy / input-construction defects. The window's evidence is that the yield arrives exactly when a security surface is in the diff and never otherwise, which is the conditional's predicate firing — so the conditional keeps the yield and drops 39 no-surface dispatches. |
| P-4 | `test-coverage-reviewer` | **demote** | 41 dispatches, 0 blockers, 6 non-blocker findings, **10 dark** — 24.4%, and 300 of the corpus's 390 wasted turns | Test-adequacy of a changed guard. Real but never blocking here, and the most expensive panelist per unit of yield in the corpus. Demotion keeps it available on diffs that actually add or change tests; see the emit-deadline mitigation below, which applies whatever the dispatch decision is. |
