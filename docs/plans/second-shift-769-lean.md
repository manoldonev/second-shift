...
- **AC-11** — `plugins/dev-pipeline/workflows/code-review.mjs`'s mitigation-ladder **comment**,
  which today describes the dark-reviewer backstop as "a NOTE", states the amended contract
  instead: that its own one-shot retry is bit-identical, that the session must re-dispatch once
  with a changed prompt, and that a surviving gap is a note for most reviewers but a hard No for
  security and a void for armed-spec fidelity. **Comment only — no executable line changes**,
  which is what keeps S-10 intact. *Added after the spec was first committed*: the edit made this
  comment stale, and the repo's rule is that a change which stales a doc needs an explicit `AC-n`
  rather than a silent fix.

## Out of scope

- `plugins/dev-pipeline/workflows/code-review.mjs`'s **executable** surface — the substrate
  already carries the numbered deadline; this ticket changes no substrate code (S-10). AC-11
  touches only the explanatory comment above the dispatch ladder.
