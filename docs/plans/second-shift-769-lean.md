...
| D-2  | What makes the re-dispatch prompt distinct, given the substrate branch already carries a numbered emit deadline | Two requirements. A turn-numbered emit deadline as a FLOOR, and narrowing to the dark reviewer's domain with the diff context the session already holds. The deadline alone is a no-op for the seven generic-branch reviewers, so narrowing is the session's actual edge                                                                                                                                 | user-answered    |
...
| D-8  | How the armed-spec fidelity escalation is expressed in Step 4b-void                                             | Widen existing case 2 to cover a fidelity reviewer that dies AFTER dispatch, not a third case. Case 2 is currently pre-dispatch only, stated in `plugins/review-toolkit/skills/review-lead/SKILL.md`. Same dimension, same armed condition                                                                                                                                                               | codebase-derived |
| D-9  | Whether `review-lean` step 5c needs a wording change                                                            | No. `plugins/dev-pipeline/skills/review-lean/SKILL.md` already reads "the provider's mandatory fidelity reviewer went dark, however many of the others returned". D-8 makes `review-lead` finally match what 5c already asserts                                                                                                                                                                          | codebase-derived |
...
| D-13 | Whether this ticket is `harness-internal`                                                                       | No. `review-lead` is a shipped consumer-facing skill and this changes what a consumer's review round does, so the label's "Ratified:" precondition does not apply                                                                                                                                                                                                                                        | codebase-derived |
