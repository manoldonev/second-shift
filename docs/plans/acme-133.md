...
3. **Repo stack context block (line 16–18)** — promote the existing weak "Repo context (load if present)" into the `db-reviewer.md:14` *load first* shape, carrying the conservative-inference fallback that lowers confidence. Leave the per-reviewer extension block (line 28) unchanged. Per the Stage-4 warnings: name the **`## UI stack & design system`** section explicitly (`plugins/review-toolkit/scripts/section-catalog.txt:26` registers `a11y-reviewer` as one of its two readers, and those names are the exact keys reviewers match on), and scope the requested dimensions to what `docs/extension-points.md:77` actually advertises — UI framework + design-system/primitives package — rather than also asking for template syntax and styling system, which that section never tells a consumer to write.
