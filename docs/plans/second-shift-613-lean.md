...
| `lint` | well-formedness / expiry / run-binding over a record or the register |
...
1. `LEAN_ATTEND_MODE=headless` → `headless (marked-headless)`. Any **other** value is an
   environment error (rc 2): `attended` is deliberately not self-assertable — a session that
   could assert its own attendance could turn off its own gates.
2. no `CLAUDE_CODE_SESSION_ID` → `headless (no-session-identity)`.
3. no token at `<mainRoot>/<pipelineStateDir>/attend-<session-id>.token` → `headless (no-token)`.
4. token unreadable, or carrying no `session_id:` → `headless (corrupt-token)`.
5. token's `session_id:` differs from the ambient one → `headless (session-mismatch)`.
6. token's `run_id:` and the ambient `RUN_ID` both present and different →
   `headless (run-mismatch)`.
7. otherwise `attended`.

Staleness is **structural** — the binding is to run identity, and there is no wall-clock TTL,
which would import the BSD/GNU `stat`/`date` and bash-3.2 portability hazards this repo has been
bitten by. A scheduler-spawned `claude -p` payload gets a fresh session id, so an operator's
token can never read attended inside one; rule 1 is the independent second belt.
...
run_id: <id, or `none` when no run id was ambient at record time>
session_id: <id>
expiry: run
...
persistent_: the override is good for the issue it names and dies with that issue's PR. Authority
is the closed `scope` enum — never an identity; actor identity comes from the commit and PR trail.

**Central register** — `.claude/lean-overrides.tsv`, for the rare persistent class only. A single
...
- `spawn()` marks its payloads headless (`LEAN_ATTEND_MODE=headless` alongside the existing
  `env -u RUN_ID -u LEAN_RUN_MODEL` scrub).
...
`lean-evidence.sh` gains an `override` arm (portable, tracker-agnostic, so it belongs to the
payload rather than to the github-only wrapper); `scripts/check-lean-chain.sh` delegates to it.
The arm validates **every present** override record — well-formedness, expiry, run binding — and
refuses a malformed or expired one. Absence is class (a): most runs record no override.
...
| --- | --- | --- | --- |
| D-1 | Where the attendance binding lives | Session id is the required binding, run id an additional one when ambient; a spawned session gets a fresh session id, which is what makes staleness structural, and no run id is ambient at scheduler preflight | codebase-derived |
| D-2 | Whether `LEAN_ATTEND_MODE=attended` is honored | No — only `headless` is honored, any other value is an environment error; an honored positive value would be the self-asserted attendance the epic forbids | codebase-derived |
| D-3 | One binary or a reader copied into each consumer | One binary invoked as a subprocess, with a closed gate enum; two parser copies would be the dual-declaration smell, and a closed enum is what keeps it private to the gate rather than a public seam | codebase-derived |
| D-4 | Record file shape | One file per issue, N blocks with per-block first-match keys; a single header cannot bind two gates in one run | codebase-derived |
| D-5 | Where a pre-lane override record is written | Into the repo root of the caller, with a printed carry-forward line when that root is not the lane branch; automating the carry would change `entry`'s behavior, which AC-6 forbids in this slice | user-delegated |
| D-6 | Expiry grammar for the persistent register | The single form naming a tracking issue, evaluated as that issue's open/closed state; a wall-clock date would import the portability hazards the token design already declined | codebase-derived |
| D-7 | Which side of the merge boundary validates the record | The portable payload, since the record is tracker-agnostic; the github-only wrapper only delegates | codebase-derived |
| D-8 | Exit code for the resumable reject | A new code, applied when the unintaken probe is the only failure, so an operator and a wrapper can both distinguish resumable from terminal without parsing prose | user-delegated |
