...
- `plugins/dev-pipeline/skills/run/stage8-perrepo-review-selftest.sh` — **3 direct completion sites migrated** via `stage_evidence` (completes stages 1–8 directly; was missing from the original blast radius)
...
6b. **Leg ordering is load-bearing (the slice-2 analog of AC-24):** within every `stage_completion_preconditions()` case arm the new legs are appended AFTER the existing legs, and stages 3/7/9's §1 writer-naming legs come BEFORE their receipt leg. Non-force refusal reports the FIRST fired guard, so this ordering keeps every existing `sct_err` message assertion byte-stable, and the new `(l3)`/`(l7)`/`(l9)` cases assert the §1 writer-naming message rather than the receipt message.
...
Shell repo, no `unitTestScope` (mutation gate off). Per-invocation invariants → `statectl-selftest.sh` (each new case carries its scenario-first justification comment); the composed jira-adapter zero-evidence path and the receipt-bearing full walk → `scenario-liveness-selftest.sh`; `e2e-replay-selftest.sh` re-runs unmodified as the fixture-migration regression proof. AC-21 is a re-verification only (already satisfied by #246).
