...
- `plugins/dev-pipeline/skills/run/tools/check-config-shadowing.sh` — the `stages/1-intake.md|tracker.branchPrefix|Stage-1 branch prefix` pin row. **Added during implementation, not planned**: the pin asserts Stage 1 *reads* `tracker.branchPrefix`, and the outer-loop deletion removes its only reader. The pin's premise is obsolete — after the retirement Stage 1 derives no branch at all, so branch-prefix consumption belongs entirely to Stage 2, whose own pin row (`stages/2-worktree.md|tracker.branchPrefix`) stays and still passes. Removing the row is the correct fix; re-adding a read to Stage 1 to satisfy it would be inventing a consumer
...
7. `e2e-replay-selftest.sh`: delete the `$MAXSLICE` resolution + preflight and **all of scenario 4**, including leg `(sl4)`; update the header's scenario map. (Ordering change from this plan's first draft, which put `(sl4)` in commit 2: scenario 4 is one contiguous block and `(sl4)` is equally stacked-only, so splitting it would leave half a scenario. Safe in this direction — removing a caller early can never break an exit-99 preflight, only the reverse can. The fixture `stacked-prs.json` still goes in commit 2.)
