...
| D-6 | four spawns landed on already-merged issues — a scheduler defect that is not transport | Class **M** (mis-dispatch) added and reported separately rather than folded into X, where it would have been invisible. Neutral to `M1ᵗ`. The class stands; two of the four rows first assigned to it do not — corrected by `D-9`. | codebase-derived |
...
| D-9 | two of class M's four rows are contradicted by the logs they cite | Re-derived from the spawn logs, the gate's progress records and the tracker: `533-lean-spawn-1` entered at 16:29:11Z, 66 minutes BEFORE PR #556 merged, and walked milestones 1-4 to satisfied before the close; `530-lean-spawn-4` says in its own first paragraph that a concurrent run completed underneath it mid-command, corroborated by its progress block straddling the close. Both reclassified **clean**; `530-5` and `549-6` stay M because each opens by reporting a closure it found on arrival. Recorded with its direction because dropping rows out of M moves the naive reading TOWARD keep, the arm this session predicted against. | user-answered |
| D-10 | the decision table is two-valued at exactly M1 transport = 0.80 | The B band becomes half-open, 0.50 <= M1 < 0.80, so exactly 0.80 falls to the two `>= 0.80` rows. Operator ruling, and the only reading consistent with `>= 0.80` appearing inclusive and unchanged in every revision since revision 1. Landed as an APPENDED revision 4 (`R4-1`), never as an edit to `R3-2` - the append discipline is what keeps a moving bar visible. Direction: favours the scheduler. | user-answered |
| D-11 | round-1 B-6 / round-2 B-8, the three dependents sequenced behind a ticket that no longer owns the decision | Discharged by the operator outside this branch: re-point comments now exist on #617, #638 and #639 pointing the gate at #650. No branch change was required and none was made. The links are carried in the handoff so round 3 verifies without hunting. This session posted no tracker comment. | user-answered |
| D-12 | the uncalled `lean-gate.sh:2502` staleness refusal that D-9 exposed | Routed to **#650** as follow-up material and named in the audit's limitation 3; NOT wired up here. Wiring it would add shell and a gate call to a slice whose "no shell, no gate, zero guard mass" claim is load-bearing for the #641 check. The evidence belongs in this file; the remedy belongs to the campaign ticket. | user-answered |
