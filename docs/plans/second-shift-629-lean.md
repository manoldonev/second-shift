...
| D-11 | OR-1 resolved — the allowance is 20 percent, over a 106s baseline | The un-deferred set has changed since 2026-08-20 (67 suites discovered then, 73 now), so the seed was re-taken through the new emitter rather than inherited. Three consecutive measurements on the enforcing lane came in at 113s, 96s and 108s — a spread of about minus 9 to plus 7 percent around their mean, which is what a hosted runner costs. 20 percent is roughly twice that spread, so ordinary noise does not flap it. The cost is stated rather than hidden: the aggregate arm resolves at two to three threshold-crossing suites, not one, and a single arrival is AC-3's named warning instead. Tightening below the lane's own noise would buy a red morning and nothing else. A reversible default, changed in one reviewed commit. | user-delegated |
