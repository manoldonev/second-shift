...
9. **Update `cost-tracking-fixtures/README.md`** — lines 19-20's expected-outcome bullets become config-conditional (bot enabled + no wrapper → `skipped-no-bot-wrapper`; bot disabled → reaches the amend step under operator identity, then fails the fake-URL parse → `skipped-amend-failed`).
