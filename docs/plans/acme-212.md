...
2. Add `require_verify_sidecar <key> <current-state-json>` `[NEW]` alongside it. It reads `runId` from the in-memory state, derives the sidecar directory from `state_path`, and branches exactly like the neighbouring `verifySummary` check: per-target `{key}-<id>-verify.json` for every entry of `targetRepos`, else the flat `{key}-verify.json`. Each refusal `die`s naming `verifyctl.sh` as the required producer, distinguishing absent-vs-stale, and ending in the house `--force for crash-recovery` phrasing.
...
4. Add `write_verify_sidecar <key> [repo-id]` `[NEW]` to `scenario-lib.sh`: read `runId` back from state via `sct get`, and write a plausible sidecar (`runId`, `headSha`, `chargedHead`, `at`, empty `failedClasses`, `status: "pass"`) to the fixture state dir. It must run **after** `reset_state`, which deletes `*.json` from that dir.
...
8. Extend the `no-split` scenario in `scenario-liveness-selftest.sh` with a non-vacuity check: the same full-green run, with the Stage-6 sidecar removed, must fail to complete Stage 6 — so the scenario cannot stay green if the precondition is deleted. This mirrors the existing `(ns3)` non-vacuity check for the stage-9 receipt.
9. Update `state-schema.md`: add the attestation clause to the Stage-6 row of the completion-evidence table, and extend the verifyctl-sidecar note with the gate's dependency on it, the `--no-attempt` non-attestation rule, and the declared stacked-run limitation from D-2.
...
| AC-4 | Selftest covers refusal + pass path; `no-split` scenario composes against the precondition | 4, 5, 6, 7, 8 | `statectl-selftest.sh` new block; `scenario-liveness-selftest.sh` `no-split` leg + non-vacuity check (AC-4) |
