...
| 9 | c | #668 | label `sonnet`; **actual model NOT CAPTURED.** An attended session inherits whatever model it runs in and arm c cannot bind the label (rows 7 and 8 both ran **opus** under `sonnet` labels). This row ran on a second machine and its model was not recorded there, so **arm c's internal model consistency is UNDETERMINED**. Single-PR (triplet T3, bound 2026-08-25) | ≤2026-08-25T17:30:36Z (first branch commit; the ledger that would give the launch keystroke is machine-local to the run machine and was not captured) | 0:31:07 to first verdict (r1 needs-work 18:01:43Z, per protocol item 3); terminal approve 20:36:23Z at **round 2**, merged 20:45:48Z, PR #685. Contains a 2h18m gap (18:18:20Z→20:36:23Z) **not confirmed as off-shift**, so no shift exclusion is applied | 4 | **NOT CAPTURED** — `668-lean-launches.tsv` is machine-local to the run machine. The 4 sessions (2 build, 2 review) are derived from the one-per-round verdict records and the build commits, the same reconstruction row 8 required | **~25 — RECONSTRUCTED BY ANALOGY, not measured.** Row 7 is the same arm at the same round count and cost ~25; there is no first-hand basis for this row. Operator confirmation owed at scoring | none by rubric; two structural gaps recorded in the intervention detail | machine-local to the run machine, not captured |
...
| 9 | none (structural, not an intervention) | Two evidence gaps, both structural rather than failures of the run. **(a)** The model the attended session ran on was never recorded. Arm c inherits the session model and the scheduler stamps `--build-model` regardless, so the ledger would have said `sonnet` either way; rows 7 and 8 are known to have run opus, and this row is unknown, which leaves the arm's three rows possibly mixed with no way to settle it after the fact. **(b)** The run happened on a second machine, so its launch ledger never reached this repo — the invocation count and launch keystroke time are unrecoverable, exactly the exposure run 2 had and the runbook warned about for this row. Neither affects the approve; both bound what arm c's rows can be compared on. | PR #685 commits 44005e0d→7d33d86f; `668-lean-launches.tsv` absent from `.claude/pipeline-state/` on the machine holding the corpus |
