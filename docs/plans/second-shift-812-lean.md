...
| D-19 | The `--issue` input to `score` | The results row's 19-column contract carries no issue number and a cell id (`t4-control-r2`) encodes the ticket ROLE rather than the substrate issue, so D-4's PR resolution on `<branchPrefix><issue>` has no in-band source for it. `score` takes it as a required `--issue <n>` flag, validated numeric, rather than deriving it from the cell id or adding a 20th column. This ADDS a flag to the CLI shape the issue body declared, which is why it is recorded here rather than left implicit | codebase-derived |
