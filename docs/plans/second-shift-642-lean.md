...
- **AC-6** (proxy): combined `lean-gate.sh` + `lean-gate-selftest.sh` line count drops by at
  least 30% against this branch's merge-base, and `scripts/check-guard-budget.sh origin/main`
  reports a negative guard/test mass delta (D-1 — this replaces the ticket's
  `tools/guard-budget.tsv` clause, whose subject #641 deleted).
