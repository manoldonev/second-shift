...
| D-7 | Where enforcement runs | Both layers, the pattern #613 already shipped. `lean-gate.sh verdict` refuses to WRITE a self-contradictory record, so the reviewer learns in-session before the round is spent; `lean-evidence.sh` — which `check-lean-chain.sh` already delegates its verdict arms to — validates the committed record at the merge boundary. DEPARTURE on one clause: the receipt read "covering CI and milestone 4 together", and milestone 4 does not call `lean-evidence.sh` — #720 deleted its duplicate arms and made the boundary the sole reader. So the two layers are the writer and the boundary; milestone 4 gains nothing, which is #720's rule, not a narrowing of this one. | user-answered |
...
| D-10 | Disposition of the censused prose `pb-0426581f` once the gate ships | Delete the blocker sentence in `plugins/dev-pipeline/skills/review-lean/SKILL.md` (the sentence beginning **"Approve on the diff, not on the spec's promises"** — cited by TEXT, not by line: this anchor has moved three times, so a pinned number is stale before BUILD reads it) per P5. Keep and EXTEND the scoring instruction with the `divergent-inert` value and its measurement-and-follow-up requirement. The triage row moves to `promoted \| guard-added \| <enforcer key>`. DEPARTURE on the deletion's EXTENT: only the unmet-AC clause goes, because the amended-spec clause is #753's construct and deleting it here would leave that rule with neither prose nor gate. | user-answered |
