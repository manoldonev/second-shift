...
- **AC-7** — no documentation is left stale by this change. The `N/A` behaviour is restated at the
  ...
  discharge the near-misses it turns up, and they come from the **broader `reached nothing`
  sweep** D-13's row names, not the literal `N/A` sweep — neither line contains `N/A`.
  `lean-gate-selftest.sh:3987` and `scenario-liveness-selftest.sh:1370` state the pre-#694
  condition in the past tense, so they are correct as they stand.
