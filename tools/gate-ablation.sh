...
# The OPERATOR names them, via `--exclude`: the lane registry this used to read as a second source
# was retired with milestone 3's supervision stratum (#566). A lane left unnamed lands in the
# corpus and shows up later as `emit`'s drift refusal on its record, which is the loud failure.
#
# Usage:
#   gate-ablation.sh manifest [--state-dir <dir>] [--exclude <ids>]
...
if [ "$SUB" = manifest ]; then
  # `--exclude` IS THE EXCLUSION SOURCE, and the only one. It was the operator's half of a pair
  # until #566 retired the lane registry that supplied the other half — an automatic in-flight
  # read this tool can no longer make, and a `from the lane registry: none` header line rendered
  # identically whether nothing was in flight or nothing could be read. So the operator names the
  # live lanes or nobody does, the header says so in as many words, and an in-flight record that
  # slips into the corpus surfaces as `emit`'s drift refusal naming it rather than as silence.
  live="$(echo "$EXCLUDE" | tr ',' ' ' | tr ' ' '\n' | grep -E '^[0-9]+$' | sort -n -u | tr '\n' ' ' | sed 's/[[:space:]]*$//')"
  ...
  echo "# excluded, still in flight when this was cut, named by --exclude: ${live:-none}"
