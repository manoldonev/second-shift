#!/usr/bin/env bash
# lane-bench-arm.sh — the lane bench's arm wrapper. Used as `LANE_SPAWN_BIN`, it loads one
...
# the bench substitutes for the session binary. `orchestrate.sh` routes EVERY control-plane
...
# `LANE_RUN_MODEL` and the forwarded `SECOND_SHIFT_CONFIG`, and a second one here would replace it
...
#
#   LANE_ARM_MANIFEST   required. One plugin directory per line, absolute. Written per cell by
#                       `lane-bench.sh run`; there are no checked-in manifests.
#   LANE_ARM_ADD_DIRS   optional. One absolute directory per line, appended as `--add-dir` on a
...
#   LANE_ARM_ALLOWED_TOOLS  optional. One permission rule per line, passed as `--allowedTools` on a
...
MANIFEST="${LANE_ARM_MANIFEST:-}"
[ -n "$MANIFEST" ] || die "LANE_ARM_MANIFEST is unset — this wrapper exists to load an arm's plugin directories and has none to load. Set it, or use the session binary directly."
[ -f "$MANIFEST" ] && [ -r "$MANIFEST" ] \
  || die "LANE_ARM_MANIFEST names no readable file: $MANIFEST"
...
[ "${#ENTRIES[@]}" -gt 0 ] || die "LANE_ARM_MANIFEST is empty: $MANIFEST — an arm with no plugin directory is not the skeleton, it is no kit at all"
...
if [ "$DISPATCH" -eq 1 ]; then
  ...
  # cwd — so the dry run ended `blocked` with no PR. `LANE_ARM_ADD_DIRS` names the directories to
  ...
  if [ -n "${LANE_ARM_ADD_DIRS:-}" ]; then
    while IFS= read -r d; do
      [ -n "$d" ] && set -- "$@" --add-dir "$d"
    done <<< "$LANE_ARM_ADD_DIRS"
  fi
  ...
  # `LANE_ARM_ALLOWED_TOOLS` names permission rules to allow, one per line, passed as one
  # `--allowedTools` on a dispatch and on nothing else — narrow rules for the writes the gate
  # must make, identically in every arm.
  if [ -n "${LANE_ARM_ALLOWED_TOOLS:-}" ]; then
    set -- "$@" --allowedTools
    while IFS= read -r t; do
      [ -n "$t" ] && set -- "$@" "$t"
    done <<< "$LANE_ARM_ALLOWED_TOOLS"
  fi
  ...
  if [ -n "${LANE_ARM_DISALLOWED_TOOLS:-}" ]; then
    ...
    while IFS= read -r t; do
      [ -n "$t" ] && set -- "$@" "$t"
    done <<< "$LANE_ARM_DISALLOWED_TOOLS"
  fi
fi
