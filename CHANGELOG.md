...
- **review-lead's default panel on the pipeline path: scope-completeness only, the rest opt-in (#841)** (#841)
  ...
  Migration: none.
- **fix(dev-pipeline): the readyProbe retries, reports what it saw, and charges nothing (#845)** (#845)
  The armed render's readyProbe now takes three readings instead of one, reports the HTTP status or curl exit it actually saw, and no longer spends a milestone-3 fix attempt when the prerequisite is unreachable.
